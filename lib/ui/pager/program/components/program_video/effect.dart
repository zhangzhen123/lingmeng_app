...
//    ProgramVideoAction.action: _onAction,
    ...
    ProgramVideoAction.replay: _didReplay,
  });
}
...
Future<void> _init(Action action, Context<ProgramVideoState> ctx) async {
  ...
  var url = CommonUtils.getPlayUrl(ctx.state.video?.playInfo);
...
Future<void> _didReplay(Action action, Context<ProgramVideoState> ctx) async {
  debugPrint('ProgramVideoState _didReplay =${ctx.state.video}  sta=${ctx.state.hashCode}');

//  ctx.state.ijkController = IjkMediaController();
  var url = CommonUtils.getPlayUrl(ctx.state.video?.playInfo);
//  url = 'rtmp://aliyun-rtmp.51lm.tv/lingmeng/27536';
  debugPrint('ProgramVideoState _didReplay playInfo=$url ijk=${ctx.state.ijkController.hashCode}');
  await ctx.state.ijkController.pauseOtherController();
  debugPrint('ProgramVideoState _didReplay pauseOtherController');
  await ctx.state.ijkController.setNetworkDataSource(url, autoPlay: true);
  debugPrint('ProgramVideoState _didReplay 设置源成功');
//  await ctx.state.ijkController.play();
//  debugPrint('开始播放成功');
}
